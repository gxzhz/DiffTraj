...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 拿到权重
        weights = self.fc(x)  # weights shape: (batch_size, num_attributes, 1)
        # apply softmax along the attributes dimension
        # 对权重张量应用 softmax 函数，沿着属性维度（dim=1）进行归一化处理。
...
class WideAndDeep(nn.Module):
    ...
    def __init__(self, embedding_dim=128, hidden_dim=256):
        ...
        # 设置三个嵌入层
        self.depature_embedding = nn.Embedding(1024, hidden_dim)
    ...
    def forward(self, attr):
        # Continuous attributes连续属性
        continuous_attrs = attr[:, 1:6]

        # Categorical attributes分类属性
...
class Upsample(nn.Module):
    # 默认在上采样后应用一个卷积层（with_conv），输入通道数（in_channels）
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, temb):
        # 在 forward 方法中，首先对输入张量进行归一化操作，并应用 Swish 激活函数。
        # 然后通过一个 1D 卷积层 self.conv1 对输入进行卷积处理。接着对温度嵌入进行投影，并将其与卷积结果相加。
        # 再次进行归一化、Swish 激活函数、Dropout 和卷积操作，得到残差块的输出。
        h = x
        h = self.norm1(h)
        h = nonlinearity(h)
        h = self.conv1(h)
